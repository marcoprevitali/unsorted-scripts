...
TARGET_PRESSURE = 1000.0 # Pa, the confining pressure
...
# Maccaferri lists characteristic EA=500 kN/m for 8x10, 2.7 mm gabion mesh.
# The corresponding 120-year design EA in gravel is 398.1 kN/m.
MACCAFERRI_MESH_AXIAL_STIFFNESS = 500.0e3 # N/m; source: https://www.maccaferri.com/wp-content/uploads/2023/02/Tensile-resistance-and-axial-stiffness.png
MACCAFERRI_MESH_WIRE_DIAMETER = 2.7e-3 # m; source: https://www.maccaferri.com/uk/products/gabions/
MACCAFERRI_SELVEDGE_WIRE_DIAMETER = 3.4e-3 # m; source: https://www.batiproduits.com/documentation-technique/gabion-double-torsion-54768-12879560.html

# Convert mesh EA into PFC pb_kn [Pa/m]. One modeled strand represents a
# strip of mesh of width MOS, and pb_kn*A*L is its represented axial EA.
MESH_PARALLEL_BOND_AREA = ma.pi * NET_BALL_RADIUS**2
MESH_REPRESENTED_WIRE_EA = MACCAFERRI_MESH_AXIAL_STIFFNESS * MOS
map_mesh_tensile_stiffness = {
    "sw": MESH_REPRESENTED_WIRE_EA / (SW_DIST * MESH_PARALLEL_BOND_AREA),
    "dt": MESH_REPRESENTED_WIRE_EA / (DT * MESH_PARALLEL_BOND_AREA),
    "se": (
        MESH_REPRESENTED_WIRE_EA
        * (MACCAFERRI_SELVEDGE_WIRE_DIAMETER / MACCAFERRI_MESH_WIRE_DIAMETER)**2
        / ((MOS / 2.0) * MESH_PARALLEL_BOND_AREA)
    ),
}
MESH_COMPRESSIVE_STIFFNESS_RATIO = 0.70

# optional mesh pretension applied before the lateral walls are removed
MESH_PRETENSION_ENABLED = True
MESH_PRETENSION_STEPS = 10
MESH_PRETENSION_CYCLES_PER_STEP = 200
MESH_PRETENSION_CALM = 50
MESH_PRETENSION_FIX_SELVEDGE = True
MESH_PRETENSION_BASE_FRACTION = 0.75
...
# Apply the same wire-type stiffnesses used during the reload compression test.
map_updated_mesh_contacts = {contact_type: 0 for contact_type in map_mesh_tensile_stiffness}
for contact in it.contact.list(type=it.BallBallContact, all=True):
    if contact.model().lower() != "linearpbond":
        continue

    mesh_contact_type = None
    for contact_type in map_mesh_tensile_stiffness:
        if contact.in_group(contact_type):
            mesh_contact_type = contact_type
            break
    if mesh_contact_type is None:
        continue

    tensile_stiffness = float(map_mesh_tensile_stiffness[mesh_contact_type])
    pb_radius = float(contact.prop("pb_radius"))
    pb_area = ma.pi * pb_radius**2
    compressive_stiffness = (
        MESH_COMPRESSIVE_STIFFNESS_RATIO * tensile_stiffness * pb_area
    )
    ...
    map_updated_mesh_contacts[mesh_contact_type] += 1
it.command("model clean all")

# Set the final fill/net/support contact models before any pretension cycling.
...
# Count the wires crossing each box midplane and set their axial tension so
# that the resultant force divided by the orthogonal face area is TARGET_PRESSURE.
# The first local component of pb_force is negative in tension.
if MESH_PRETENSION_ENABLED:
    if MESH_PRETENSION_STEPS < 1:
        raise RuntimeError("MESH_PRETENSION_STEPS must be at least one")

    vec_midplane = np.array((
        0.5 * (mx0 + mx1),
        0.5 * (my0 + my1),
        0.5 * (mz0 + mz1),
    ), dtype=float)
    box_size_x = max(current_x_max - current_x_min, 1.0e-12)
    box_size_y = max(current_y_max - current_y_min, 1.0e-12)
    box_size_z = max(float(settled_top_wall_z), 1.0e-12)
    vec_orthogonal_face_area = np.array((
        box_size_y * box_size_z,
        box_size_x * box_size_z,
        box_size_x * box_size_y,
    ), dtype=float)
    vec_target_section_force = TARGET_PRESSURE * vec_orthogonal_face_area

    pretension_contacts = []
    vec_wire_crossing_count = np.zeros(3, dtype=int)
    vec_effective_wire_count = np.zeros(3, dtype=float)
    vec_axis_wire_crossing_count = np.zeros(3, dtype=int)
    vec_axis_effective_wire_count = np.zeros(3, dtype=float)

    for contact in it.contact.list(type=it.BallBallContact, all=True):
        if contact.model().lower() != "linearpbond":
            continue
        if not any(contact.in_group(contact_type) for contact_type in map_mesh_tensile_stiffness):
            continue
        if int(contact.prop("pb_state")) != 3:
            continue

        vec_position_1 = contact.end1().pos()
        vec_position_2 = contact.end2().pos()
        vec_position_1 = np.array(tuple(float(value) for value in vec_position_1))
        vec_position_2 = np.array(tuple(float(value) for value in vec_position_2))
        vec_link = vec_position_2 - vec_position_1
        link_length = float(np.linalg.norm(vec_link))
        if link_length <= 1.0e-12:
            continue

        vec_direction = np.abs(vec_link) / link_length
        aligned_axis = int(np.argmax(vec_direction))
        if (
            vec_direction[aligned_axis] < 1.0 - 1.0e-8
            or np.sum(vec_direction) - vec_direction[aligned_axis] > 1.0e-8
        ):
            aligned_axis = -1

        vec_crossing = np.zeros(3, dtype=float)
        for axis in range(3):
            coordinate_1 = vec_position_1[axis]
            coordinate_2 = vec_position_2[axis]
            plane_coordinate = vec_midplane[axis]
            if abs(coordinate_2 - coordinate_1) <= 1.0e-12:
                continue
            if min(coordinate_1, coordinate_2) < plane_coordinate <= max(coordinate_1, coordinate_2):
                vec_crossing[axis] = 1.0
                vec_wire_crossing_count[axis] += 1
                vec_effective_wire_count[axis] += vec_direction[axis]
                if aligned_axis == axis:
                    vec_axis_wire_crossing_count[axis] += 1
                    vec_axis_effective_wire_count[axis] += vec_direction[axis]

        pretension_contacts.append((
            contact,
            vec_direction,
            vec_crossing,
            aligned_axis,
    ...
    if np.any(vec_wire_crossing_count == 0):
        raise RuntimeError(
            "Could not find mesh wires crossing all three orthogonal midplanes"
        )
    if np.any(vec_axis_wire_crossing_count == 0):
        raise RuntimeError(
            "Could not find axis-aligned mesh wires crossing all three midplanes"
        )
    if not 0.0 <= MESH_PRETENSION_BASE_FRACTION <= 1.0:
        raise RuntimeError(
            "MESH_PRETENSION_BASE_FRACTION must be between zero and one"
        )

    # Apply a positive common tension to all wires, then distribute each
    # remaining face force through wires aligned with that face normal.
    # This exactly matches all three target pressures without requiring any
    # parallel bond to carry a nonphysical compressive pretension.
    common_tensile_force = (
        MESH_PRETENSION_BASE_FRACTION
        * float(np.min(vec_target_section_force / vec_effective_wire_count))
    )
    vec_residual_section_force = np.maximum(
        vec_target_section_force
        - common_tensile_force * vec_effective_wire_count,
        0.0,
    )
    vec_axis_tensile_increment = (
        vec_residual_section_force / vec_axis_effective_wire_count
    )

    target_pretension_contacts = []
    for contact, vec_direction, vec_crossing, aligned_axis in pretension_contacts:
        target_tensile_force = common_tensile_force
        if aligned_axis >= 0:
            target_tensile_force += vec_axis_tensile_increment[aligned_axis]
        target_pretension_contacts.append((
            contact,
            float(target_tensile_force),
            vec_direction,
            vec_crossing,
        ))

    vec_initial_section_force = np.zeros(3, dtype=float)
    for contact, target_tensile_force, vec_direction, vec_crossing in target_pretension_contacts:
        vec_initial_section_force += (
            vec_crossing * target_tensile_force * vec_direction
        )
    vec_initial_pressure = (
        vec_initial_section_force / vec_orthogonal_face_area
    )
    print(
        "Mesh pretension wire crossings x/y/z = "+
        str(tuple(int(value) for value in vec_wire_crossing_count))
    )
    print(
        "Mesh pretension effective wires x/y/z = "+
        str(tuple(float(value) for value in vec_effective_wire_count))
    )
    print(
        "Mesh pretension axis-aligned crossings x/y/z = "+
        str(tuple(int(value) for value in vec_axis_wire_crossing_count))
    )
    print(
        "Mesh pretension initial pressure x/y/z = "+
        str(tuple(float(value) for value in vec_initial_pressure))
    )

    if MESH_PRETENSION_FIX_SELVEDGE:
        it.command("ball fix velocity spin range group 'dt2'")

    for pretension_step in range(1, MESH_PRETENSION_STEPS + 1):
        pretension_fraction = pretension_step / float(MESH_PRETENSION_STEPS)
        for contact, target_tensile_force, vec_direction, vec_crossing in target_pretension_contacts:
            vec_pb_force = contact.prop("pb_force")
            contact.set_prop(
                "pb_force",
                (
                    -pretension_fraction * target_tensile_force,
                    float(vec_pb_force[1]),
                    float(vec_pb_force[2]),
                ),
            )
        it.command("model cycle "+str(MESH_PRETENSION_CYCLES_PER_STEP)+
                   " calm "+str(MESH_PRETENSION_CALM))

    # Reapply the exact target after the ramp because bond motion during the
    # intermediate cycles changes the incrementally stored parallel-bond force.
    for contact, target_tensile_force, vec_direction, vec_crossing in target_pretension_contacts:
        vec_pb_force = contact.prop("pb_force")
        contact.set_prop(
            "pb_force",
            (
                -target_tensile_force,
                float(vec_pb_force[1]),
                float(vec_pb_force[2]),
            ),
        )
    ...
    vec_actual_section_force = np.zeros(3, dtype=float)
    for contact, target_tensile_force, vec_direction, vec_crossing in target_pretension_contacts:
        vec_pb_force = contact.prop("pb_force")
        actual_tensile_force = -float(vec_pb_force[0])
        vec_actual_section_force += (
            vec_crossing * actual_tensile_force * vec_direction
        )
    vec_actual_pressure = (
        vec_actual_section_force / vec_orthogonal_face_area
    )
    print(
        "Mesh pretension pressure after ramp x/y/z = "+
        str(tuple(float(value) for value in vec_actual_pressure))
    )

    if MESH_PRETENSION_FIX_SELVEDGE:
        it.command("ball free velocity spin range group 'dt2'")
