...
def compute_stress():
    ...
    volume = h*w*t
    force  = itasca.ballballarray.force_global()
    branch = itasca.ballballarray.branch()
    return np.einsum('ki,kj->ij',force,branch) / (-1.0*volume)
    

targetkPa = abs(tszz)
volume_solids = itasca.fish.get('V_ball')
...
skippy = 100
def ballOrientation():
    if itasca.cycle()%skippy == 0:
        totrot = 0
        maxrot = 0
        for b in itasca.ball.list():
            rad = b.euler()#('euler')
            grad = [math.degrees(rad[0]),math.degrees(rad[1]),math.degrees(rad[2])]
            gradmag = np.linalg.norm(rad)
        totrot+=gradmag
        maxrot = max([maxrot,gradmag])
        meanrot = totrot/itasca.ball.count()
        itasca.fish.set('meanrot',meanrot)
        itasca.fish.set("maxrot",maxrot)

itasca.set_callback("ballOrientation", -1.0)
...
itasca.command("model save 'temp_"+str(int(targetkPa))+"kPa_phi_"+str(int(phi))+".sav'")
