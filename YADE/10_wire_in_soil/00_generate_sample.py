...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    phi = utils.voxelPorosity(end=(box_size,box_size,box_height))
...
#   effective porosity computation
    plot.addData(t=Omega.time,phi=utils.voxelPorosity(end=(box_size,box_size,box_height)),snf = snf, ssf=ssf,unb=unb,mac = max_accell)
    if unb<0.001:
...
def sample_bin_porosity():
    ...
    for i in range (0,num_poro_bins-1):
        for j in range (0,num_poro_bins-1): #ashtag torrette
            ll=((box_size/(num_poro_bins)*i),((box_size/(num_poro_bins)*j)),0)
            ur=((box_size/(num_poro_bins)*(i+1)),((box_size/(num_poro_bins)*(j+1))),box_height)
            k=1
...
# -----------------------------------------------------------------------------------------------------------------------
    ...
    InteractionLoop(
        ...
        [
            ...
            # used for the cohesive sphere-cylinder interaction
        ],
        [
        # Interaction phusics
        ...
        [
            ...
            Law2_ScGeom_MindlinPhys_Mindlin(label='myViscoElasticLaw')# used for the cohesive sphere-cylinder interaction
        ]
    ),
...
def generate_string():
    ...
    for i in linspace(0,L,nL):
        nodesIds.append( O.bodies.append(gridNode([box_size/2,i,box_height/2],rCyl,wire=False,fixed=False,color=[1,0,0],material='gridNodeMat') ) )

    ### Now create connection between the nodes
    for i,j in zip( nodesIds[:-1], nodesIds[1:]):
        O.bodies.append( gridConnection(i,j,rCyl,material='gridCoMat',color=[1,0,0]) )
...
#   sieve size (therefore the maximum and the minimum particle diameter)
    ...
    north = O.bodies.append(utils.wall(box_size,axis=1,material='Muro'))
    east = O.bodies.append(utils.wall(box_size,axis=0,material='Muro'))
