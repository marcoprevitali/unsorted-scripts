...
def generate_string():
    ...
    for i in linspace(0,L,nL):
        nodesIds.append( O.bodies.append(gridNode([box_length/2,i,box_height/2],rCyl,wire=False,fixed=False,color=[1,0,0],material='gridNodeMat') ) )
    ...
    ### Set fixed nodes
    O.bodies[nodesIds[0]].state.blockedDOFs='xyz'
    O.bodies[nodesIds[-1]].state.blockedDOFs='xyz'
    O.bodies[nodesIds[-1]].shape.color=[0,0,1]
    O.bodies[nodesIds[0]].shape.color=[0,0,1]
...
#   sieve size (therefore the maximum and the minimum particle diameter)
    ...
    north = O.bodies.append(utils.wall(box_width,axis=1,material='Muro'))
    east = O.bodies.append(utils.wall(box_length,axis=0,material='Muro'))
...
#   expansion cycles
    ...
    O.bodies[nodesIds[-1]].state.blockedDOFs='xyz'
    O.bodies[nodesIds[0]].state.blockedDOFs='xyz'
    O.bodies[nodesIds[-1]].shape.color=[0,1,0]
    O.bodies[nodesIds[0]].shape.color=[0,1,0]
